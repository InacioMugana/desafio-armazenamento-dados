...
const signup = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    if (!name || !email || !password || !password) {
      return res
        .status(400)
        .json({ message: "Os dados introduzidos não são válidos." });
    }

    if (userExist) {
      return res
        .status(400)
        .json({ errors: "O endereço introduzido já está registado." });
    }
    ...
    res
      .status(201)
      .json({ message: "Utilizador criado com sucesso!", _id: user._id });
  } catch (err) {
    res.status(400).json({ err: "Internal Server error." });
  }
});
